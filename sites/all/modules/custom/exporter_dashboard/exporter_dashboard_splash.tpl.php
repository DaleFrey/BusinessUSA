<div class="exporterdashboard-mastercontainer exporterdashboard-mastercontainer-splash">

    <?php
        print theme(
            'devnote', 
            array(
                'welcomeMsg' => '',
                'message' => '
                    Dev Note: The following markup is generated from the ' .  __FILE__ . ' file.
                ',
                'showControles' => 'hover'
            )
        );
    ?>

    <!-- /* Coder Bookmark: CB-VIJHGR1-BC in <?php print basename(__FILE__); ?> */ -->
    <ul class="exporterdashboard-major-container">
        <li class="exporterdashboard-major-item exporterdashboard-major-odd exporterdashboard-minor-first" id="learn-link">
            <h2>
                <a href="javascript: loadExportPage('learn'); void(0);">
                    Learn More
                </a>
            </h2>
        </li>
        <li class="exporterdashboard-major-item exporterdashboard-major-even" id="comply-link">
            <h2>
                <a href="javascript: loadExportPage('comply'); void(0);">
                    Compliance Resources
                </a>
            </h2>
        </li>
        <li class="exporterdashboard-major-item exporterdashboard-major-odd exporterdashboard-minor-last" id="seek-link">
            <h2>
                <a href="javascript: loadExportPage('assistanceandopportunities'); void(0);">
                    Seek Assistance
                </a>
            </h2>
        </li>
    </ul>
    
    <!-- /* Coder Bookmark: CB-UP5DO2U-BC in <?php print basename(__FILE__); ?> */ -->
    <ul class="exporterdashboard-minor-container">
        <li class="exporterdashboard-minor-item exporterdashboard-minor-odd exporterdashboard-minor-first">
            <a href="/begin-exporting">
                Begin Exporting
            </a>
            <span>
                Determine your business's preparedness for exporting and find the appropriate...
            </span>
        </li>
        <li class="exporterdashboard-minor-item exporterdashboard-minor-even">
            <a href="/expand-exporting">
                Expand Exporting
            </a>
            <span>
                Find the right resources to finance exporting for expanding your business.
            </span>
        </li>
        <li class="exporterdashboard-minor-item exporterdashboard-minor-odd">
            <a href="/rural-exporting">
                "Made in Rural America" Export and Investment Initiative
            </a>
            <span>
                Learn about the White House's initiative to help rural businesses find opportunities abroad.
            </span>
        </li>
        <li class="exporterdashboard-minor-item exporterdashboard-minor-even">
            <?php /*<a href="javascript: jQuery('.exporterdashboard-premastercontainer-maincontent').hide();jQuery('.exporterdashboard-premastercontainer-teachtellmeaboutexporting').show();"> */ ?>
            <a href="javascript: loadExportPage('teachtellmeaboutexporting'); void(0);">
                Learn About Exporting Basics
            </a>
            <span>
                Browse topics to understand how exporting works
            </span>
        </li>
        <li class="exporterdashboard-minor-item exporterdashboard-minor-odd exporterdashboard-minor-last">
            <a href="http://help.business.usa.gov/link/portal/30027/30030/ArticleFolder/19/Export-Answers">
                Frequently Asked Questions
            </a>
            <span>
                Read about other business questions regarding exporting on the...
            </span>
        </li>
    </ul>
    
    <!-- /* Coder Bookmark: CB-KPCDF5Y-BC in <?php print basename(__FILE__); ?> */ -->
    <div class="exporterdashboard-exportnews-container" rendersource="<?php print basename(__FILE__); ?>">
        
        <h2 class="exporterdashboard-exportnews-title">
            <a href="/export-news">
                Export News
            </a>
        </h2>
        
        <?php
            $viewHtml = views_embed_view('export_news', 'default'); 
            if ( is_null($viewHtml) ) {
                print '<b>Error: This environment does not have the export_news View. Please <a href="http://qa.business.usa.reisys.com/admin/structure/views/view/export_news/export">export this View from QA by clicking here</a>, and then <a href="/admin/structure/views/import">import the View into this environment here</a>. Remember to tick the checkbox that allows you to overwrite a View if it already exists.</b> ';
            } else {
                print $viewHtml;
            }
        ?>
        
    </div>
    
    <!-- /* Coder Bookmark: CB-CHZYHVH-BC in <?php print basename(__FILE__); ?> */ -->
    <div class="exporterdashboard-useacsevents-container" rendersource="<?php print basename(__FILE__); ?>">
        
        <div class="exporterdashboard-useacsevents-head">
            <h2 class="exporterdashboard-useacsevents-title">
                USA Export Centers & Trade Events
            </h2>
            <input type="button" class="exporterdashboard-useacsevents-locationselect-popup-trigger" value="Change Location" onclick="jQuery('.exporterdashboard-useacsevents-locationselect-popup-container').fadeToggle();" />
            <div class="exporterdashboard-useacsevents-locationselect-popup-container" style="display: none;">
                <form action="javascript: updateExportSplashViews(); jQuery('.exporterdashboard-useacsevents-locationselect-popup-container').hide(); void(0);">
                    <!-- DO NOT REMOVE LABEL FROM THIS FORM !! They are here for 508 compliance -->
                    <label for="countryselector">
                        Select a country
                    </label>
                    <select name="countryselector" id="countryselector" class="exporterdashboard-useacsevents-select" onchange=" if ( jQuery('#countryselector').val() == 'US' ) { jQuery('.zipselector-container').fadeIn(); } else { jQuery('.zipselector-container').hide(); } ">
                        <option value="pick">Pick a Country</option>
                        <option checked="checked" value="US">United States of America</option>
                        <?php foreach ( $variables['countries'] as $countryCode => $countryName ): ?>
                            <option value="<?php print $countryCode; ?>"><?php print $countryName ; ?></option>
                        <?php endforeach; ?>
                    </select>
                    <div class="zipselector-container" style="display: none;">
                        <label for="zipselector"> <!-- DO NOT REMOVE THIS LABEL FROM THIS FORM !! It is here for 508 compliance -->
                            Please give us your ZipCode
                        </label>
                        <input id="zipselector" name="zipselector" class="auto-fill-zip-code" type="text" />
                    </div>
                    <input type="submit" value="Apply" />
                </form>
            </div>
        </div>
        
        <div class="exporterdashboard-useacsevents-ajaxcontainer">
            <!-- Content within this div will be populated via JavaScript/AJAX -->
        </div>
        
        <script>
            jQuery(document).ready( function () {
                getUserZipCode( function (zip) {
                    updateExportSplashViews('USA', zip);
                });
            });
            
            function updateExportSplashViews(countryCode, zipCode) {
                
                if ( typeof countryCode == 'undefined' ) {
                    countryCode = jQuery('select#countryselector').val();
                    if ( countryCode == 'pick') {
                        alert('Please select a country before submitting this form');
                        return false;
                    }
                }
                
                if ( typeof zipCode == 'undefined' ) {
                    zipCode = jQuery('input#zipselector').val();
                    if ( jQuery.trim(zipCode) == '' || isNaN(parseInt(zipCode)) ) {
                        alert('Please enter a valid ZipCode');
                        return false;
                    }
                }
                
                var loadingHTML = '<img style="height: 35px; float: left" alt="" src="data:image/gif;base64," /><div style="line-height: 35px; padding-left: 10px; float: left;">Please wait as we search our resources...</div>';
                jQuery('.exporterdashboard-useacsevents-ajaxcontainer').html(loadingHTML);
                
                jQuery.get('/exporter-dashboard/splashviews?countryCode=' + countryCode + '&zipCode=' + zipCode, function (newHTML) {
                    jQuery('.exporterdashboard-useacsevents-ajaxcontainer').hide();
                    jQuery('.exporterdashboard-useacsevents-ajaxcontainer').html(newHTML);
                    jQuery('.exporterdashboard-useacsevents-ajaxcontainer').fadeIn();
                    jQuery('.exporterdashboard-exportingtools-container').height((jQuery('.exporterdashboard-useacsevents-container').height() - 40));
                });
            }
            
        </script>
        
    </div>
    
    <!-- /* Coder Bookmark: CB-LZNF8BK-BC in <?php print basename(__FILE__); ?> */ -->
    <div class="exporterdashboard-exportingtools-container" rendersource="<?php print basename(__FILE__); ?>">
        <h2 class="exporterdashboard-exportingtools-title">
            Exporting Tools
        </h2>
        <ul>
            <li class="exporterdashboard-exportingtools-item exporterdashboard-exportingtools-odd exporterdashboard-exportingtools-first">
                <a href="/export/nei">
                    National Export Initiative 
                </a>
            </li>
            <li class="exporterdashboard-exportingtools-item exporterdashboard-exportingtools-even">
                <a href="/export/ecr">
                    Export Control Reform
                </a>
            </li>
            <li class="exporterdashboard-exportingtools-item exporterdashboard-exportingtools-odd">
                <a href="/export/fta">
                    Free Trade Agreements
                </a>
            </li>
            <li class="exporterdashboard-exportingtools-item exporterdashboard-exportingtools-even">
                <a href="/export/cemark">
                    CE Marketing for the European Union Business Information Database
                </a>
            </li>
            <li class="exporterdashboard-exportingtools-item exporterdashboard-exportingtools-odd">
                <a href="http://bids.state.gov/">
                    System from the US Department of State
                </a>
            </li>
            <li class="exporterdashboard-exportingtools-item exporterdashboard-exportingtools-even exporterdashboard-exportingtools-last">
                <a href="/export/safeharbor">
                    Safe Harbor Frameworks
                </a>
            </li>
        </ul>
    </div>
    
    <script>
        jQuery('.exporterdashboard-useacsevents-title').bind('click', function () {
            if ( windowWidth < 768 ) {
                if ( jQuery('.exporterdashboard-useacsevents-ajaxcontainer').is(':visible') ) {
                    jQuery('.exporterdashboard-useacsevents-ajaxcontainer').slideUp();
                } else {
                    jQuery('.exporterdashboard-useacsevents-ajaxcontainer').hide();
                    jQuery('.exporterdashboard-useacsevents-ajaxcontainer').removeClass('never-show-ever-important');
                    jQuery('.exporterdashboard-useacsevents-ajaxcontainer').slideDown();
                }
            }
        });
        
        // This area shall be hidden/collapsed by default in mobile-layout
        jQuery(document).ready( function () {
            if ( windowWidth < 768 ) {
                jQuery('.exporterdashboard-useacsevents-ajaxcontainer').hide();
                jQuery('.exporterdashboard-useacsevents-ajaxcontainer').addClass('never-show-ever-important');
            }
        });
    </script>

</div>