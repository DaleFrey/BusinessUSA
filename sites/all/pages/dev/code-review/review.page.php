<?php

print '<textarea>';

$ticketToCommitInfo = codereview_getTicketNumberToCommitArray();
$ret = array();
foreach ( $ticketToCommitInfo as $jiraTicket => $commitArray ) {
    $filesChanged = codereview_getFilesChangedInCommits($commitArray);
    foreach ( $filesChanged as &$file ) {
        $file = 'http://svn.code.sf.net/p/businessusa/code/trunk/webapp/' . $file;
    }
    print chr(34) . $jiraTicket . chr(34) . ',' . chr(34) . implode("\n", $filesChanged) . chr(34) . "\n";
}
print '</textarea>';
return 1;

if ( empty($_REQUEST['ticket']) ) {
    $msg = 'Missing ticket parameter';
    print $msg;
    return $msg;
}

if ( strpos($_REQUEST['ticket'], 'BUSUSA-') === false ) {
    $msg = 'Invalid ticket number, ticket must start with "BUSUSA-" followed by a number.';
    print $msg;
    return $msg;
}

if ( intval(str_replace('BUSUSA-', '', $_REQUEST['ticket'])) === 0 ) {
    $msg = 'Invalid ticket number, ticket must start with "BUSUSA-" followed by a number.';
    print $msg;
    return $msg;
}

if (!empty($_COOKIE['jira_user']) && !empty($_COOKIE['jira_pass']) ) {

    $ticket = $_REQUEST['ticket'];
    $commits = codereview_getCommitsForTicket($ticket);
    $files = codereview_getFilesChangedForTicket($ticket);

    $commitsToFiles = array();
    foreach ( $commits as $commit ) {
        $commitsToFiles[$commit] = codereview_getFilesChangedInCommit($commit);
    }

    if ( !empty($_REQUEST['file']) ) {
        $targetFile = $_REQUEST['file'];
    } else {
        $targetFile = $files[0];
    }

    $diff = codereview_getCompiledDiffForFileAcrossTicket($ticket, $targetFile);
    $diff = str_replace('&', '&amp;', $diff);
    $diff = str_replace('<', '&lt;', $diff);
    $diff = str_replace('>', '&gt;', $diff);
    $diff = explode("\n", $diff);

    $commentsByLine = codereview_getCodeReviewComments(
        $ticket, 
        $_COOKIE['jira_user'], 
        $_COOKIE['jira_pass'], 
        $targetFile,
        true
    );
}
    
?>

<div class="codereview-mastercontainer">
    
    <div class="codereview-colorbox-storages" style="display: none !important;">
    
        <div class="codereview-colorbox-jiraauth">
            <div class="codereview-colorbox-jiraauth-inner" style="overflow: hidden; padding: 15px;">
                <form action="javascript: userLogInFormSubmitEvent(); void(0);">
                    <div style="width: 550px; padding-bottom: 15px;">
                        Javascript on this page will read ticket information from Jira, and will post 
                        comments on this ticket on your behalf. In order for this process to work 
                        however, this script will need your Jira authentication information.
                    </div>
                    <div style="clear: both;">
                        <label style="width: 150px; float: left; line-height: 40px;">
                            Jira Username:
                        </label>
                        <input class="jirauser" type="text" style="outline: 1px solid lightgray; width: 150px; padding: 0px;" />
                    </div>
                    <div style="height: 10px; width: 10px; clear: both;"></div>
                    <div style="clear: both;">
                        <label style="width: 150px; float: left; line-height: 40px;">
                            Jira Password:
                        </label>
                        <input class="jirapass" type="password" style="outline: 1px solid lightgray; width: 150px; padding: 0px;" />
                    </div>
                    <div style="height: 10px; width: 10px; clear: both;"></div>
                    <div style="clear: both;">
                        <input type="submit" value="Submit" style="float: right;"/>
                    </div>
                </form>
            </div>
        </div>
    
        <div class="codereview-colorbox-gathering">
            <div class="codereview-colorbox-gathering-inner" style="overflow: hidden; padding: 15px; max-width: 300px;">
                <img style="height: 35px; float: left" alt="" src="data:image/gif;base64," />
                <div style="line-height: 35px; padding-left: 10px; float: left;">
                    Processing...
                </div>
            </div>
        </div>
        
    </div>
    
    <?php if (!empty($_COOKIE['jira_user']) && !empty($_COOKIE['jira_pass']) ): ?>
    
        <div class="codereview-head">
            <h1>
                Code Review of 
                <a target="_blank" href="https://tracker.reisys.com/browse/<?php print $ticket; ?>" class="no-ext-frame">
                    <?php print $ticket . ': ' . codereview_getTicketTitle($ticket, $_COOKIE['jira_user'], $_COOKIE['jira_pass']); ?>
                </a>
            </h1>
        </div>
        
        <hr style="height: initial;" />
        
        <div class="codereview-body">
        
            <div class="leftside">
                
                <div class="codereview-fileslist-container">
                    <h2>
                        Changed Files:
                    </h2>
                    <ul class="codereview-fileslist">
                        <?php foreach ( $files as $file ): ?>
                            <li>
                                <a href="/dev/code-review/review?ticket=<?php print $_REQUEST['ticket']; ?>&file=<?php print $file; ?>">
                                    <span class="codereview-fileslist-dir"><?php print dirname($file); ?>/</span><span class="codereview-fileslist-filename"><?php print basename($file); ?></span>
                                </a>
                            </li>
                        <?php endforeach; ?>
                    </ul>
                </div>
                
                <div class="codereview-commitlist-container">
                    <h2>
                        Files Changed By Commit:
                    </h2>
                    <ul class="codereview-commitlist">
                        <?php foreach ( $commitsToFiles as $commit => $commitFiles ): ?>
                            <li>
                                <?php print $commit; ?>
                                <ul>
                                    <?php foreach ( $commitFiles as $file ): ?>
                                        <li>
                                            <?php print $file; ?>
                                        </li>
                                    <?php endforeach; ?>
                                </ul>
                            </li>
                        <?php endforeach; ?>
                    </ul>
                </div>
            
            </div>
            
            <div class="rightside">
                <div class="codereview-commitlist-container">
                    <h2>
                        Diff of 
                        <a id="filepath" target="_blank" href="http://svn.code.sf.net/p/businessusa/code/trunk/webapp/<?php print $targetFile; ?>" class="no-ext-frame">
                            <?php print $targetFile; ?>
                        </a>
                    </h2>
                    <div class="codereview-diff">
                        <?php $counter = 0; ?>
                        <?php foreach ( $diff as $diffLine ): ?>
                        
                            <?php
                                $thisLineNumber = $counter++;
                                $class = 'codereview-diff-line-' . ( $thisLineNumber );
                                
                                if ( substr($diffLine, 0, 1) === '+' && substr($diffLine, 0, 3) !== '+++' ) {
                                    $class .= " codereview-diff-newline";
                                } elseif ( substr($diffLine, 0, 1) === '-' && substr($diffLine, 0, 3) !== '---' ) {
                                    $class .= " codereview-diff-deadline";
                                }
                                
                                if ( !empty($commentsByLine[$thisLineNumber]) ) {
                                    $class .= ' codereview-diff-line-hascomment';
                                }
                            ?>
                            
                            <pre class="codereview-diff-line <?php print $class; ?>" onmouseover="diffLineMouseOverEvent(<?php print $thisLineNumber; ?>);" onclick="diffLineClickEvent(<?php print $thisLineNumber; ?>);"><?php print $diffLine; ?></pre>
                            
                            <div class="codereview-comments-container codereview-comments-container-<?php print $thisLineNumber; ?>" style="<?php if ( empty($commentsByLine[$thisLineNumber]) ) { print 'display: none;'; } ?>">
                                <?php $commentCount = ( empty($commentsByLine[$thisLineNumber]) ? '0' : count($commentsByLine[$thisLineNumber]) ); ?>
                                <div class="codereview-comments-innercontainer" class="comment-count-<?php print $commentCount; ?>">
                                    <?php foreach ( $commentsByLine[$thisLineNumber] as $index => $comment ): ?>
                                        <div class="codereview-comment-container <?php if ( $index + 1 === count($commentsByLine[$thisLineNumber]) ) { print 'last'; } ?>">
                                            <div class="codereview-comment-picname">
                                                <img class="codereview-comment-pic" src="<?php print $comment['avatar']; ?>" />
                                                <span class="codereview-comment-name"><?php print $comment['author']; ?></span>
                                                <span class="codereview-comment-colon">:</span>
                                                <span class="codereview-comment-date">
                                                    <?php print str_replace("\\n", '<br/>', $comment['date']); ?>
                                                </span>
                                            </div>
                                            <div class="codereview-comment-text">
                                                <?php print $comment['text']; ?>
                                            </div>
                                        </div>
                                    <?php endforeach; ?>
                                    <form action="javascript: commentFormSubmitEvent( jQuery('.comment-form-<?php print $thisLineNumber; ?>'), '<?php print $ticket; ?>', <?php print $thisLineNumber; ?>, '<?php print $targetFile; ?>'); void(0);" class="comment-form-<?php print $thisLineNumber; ?>">
                                        <div>
                                            Post or reply with a comment:
                                        </div>
                                        <textarea></textarea>
                                        <input type="submit" value="Post" />
                                    </form>
                                </div>
                            </div>
                                
                            
                        <?php endforeach; ?>
                    </div>
                </div>
            </div>
            
        </div>
    
    <?php endif; ?>
    
</div>
