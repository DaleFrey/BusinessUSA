<?php if ( !empty($_GET['ccontent']) ) { ?>
    <?php
        ...
        if(!empty($targetsHeaders['X-Frame-Options']) && stripos($targetsHeaders['X-Frame-Options'], 'SAMEORIGIN')) {
            header('Location: ' . $targURL);
            print "<script> document.location = '$targURL'; </script>";
            exit();
        } elseif( !empty($targetsHeaders['X-Frame-Options']) && stripos($targetsHeaders['X-Frame-Options'], 'SAMEORIGIN') === false){
            print "<script> document.location = '$targURL'; </script>";
        }
    ?>
...
<html xmlns="http://www.w3.org/1999/xhtml" xml:lang="en" version="XHTML+RDFa 1.0" dir="ltr"
  ...
  xmlns:xsd="http://www.w3.org/2001/XMLSchema#">
    <frameset resizable="no" rows="50px,*" border="0" frameborder="0" onload="loadEventHandeler();">
