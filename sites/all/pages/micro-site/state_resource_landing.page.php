
<!-- ****************   Main Content   **************** -->





<div class="microsite-sections-container">
<div class="sections-message">
    For <?php print acronymToStateName($_GET['state'])?>, BusinessUSA has compiled the following resources to help small businesses and exporters in <?php print acronymToStateName($_GET['state']); ?>.  BusinessUSA will continue to update this page with resources, business assistance centers, and events in <?php print acronymToStateName($_GET['state'])?>.  Bookmark this page to refer to it later.
</div>

<?php


print views_embed_view('state_resource_view', 'state_resource_all_content', '%'.strtolower(acronymToStateName($_GET['state'])).'%', ucfirst(acronymToStateName($_GET['state'])) , strtoupper($_GET['state']));
?>

</div>

<!-- ****************   Side Bars   **************** -->



<div class="microsite-sidebars-container">

      <!--  ****************    Top Web Resources Side Bar   ***************************** -->

    <div class="microsite-sidebar sidebar-top-web-resources">
        <div class="microsite-sidebar-title-container">
            <div class="microsite-sidebar-title">State websites for Businesses</div>
        </div>
        <div class="microsites-sidebarresults-results-container">
            <?php

            $topweb = ya_wizard_excelToArray(overridable('sites/all/pages/micro-site/state_web_sites.xlsx'));
            // $resultMarkupArray = array();
            $fullstatename = acronymToStateName($_GET['state']);
            $foundMatch = false;


            foreach ( $topweb as $row ) {

                $rowAssoc = $row['assoc'];


                if (strtolower(trim($rowAssoc['States'])) === strtolower($fullstatename))
                {
                    $foundMatch = true;
                    print '<a href='. $rowAssoc['Url']. '>' . $rowAssoc['Departments'].'</a>';

                    // $resultMarkupArray[] =
                    //   '<a href='. $rowAssoc['Url']. '>' . $rowAssoc['Departments'].'</a>';
                }
            }

            if ($foundMatch === false)
             {
                 print '<div class="view-empty-top-web">No Results found </div>';
             }


            /* foreach($resultMarkupArray as $singleresult)
             {

                 print $singleresult;

             }*/


            ?>
        </div>
    </div>

    <!-- ****************   Events Sidebar  **************** -->

    <div class="microsite-sidebar sidebar-events">
        <div class="microsite-sidebar-title-container">
            <div class="microsite-sidebar-title">Events</div>
        </div>
        <div class="microsites-sidebarresults-results-container">
             <?php print views_embed_view('state_resource_view', 'state_resource_events', $_GET['state'] ); ?>
        </div>
    </div>





    <!-- ****************    Resource Centers Sidebar  **************** -->
    <div class="microsite-sidebar sidebar-resource-centers">
        <div class="microsite-sidebar-title-container">
            <div class="microsite-sidebar-title">Resource Centers</div>
        </div>
        <div class="microsites-sidebarresults-results-container">
            <form class="begex-useac-askingforzip-inputokbutton-container" style="overflow: hidden; padding-bottom: 25px;" onsubmit="getUseacViewForZipCode( jQuery('.begex-useac-askingforzip-input').val() ); return false;">
                <div>
                    Please enter your zip code to locate the closest resource centers near you and connect with them.
                </div>
                <div class="begex-useac-askingforzip-input-container" style="float: left;">
                    <input class="begex-useac-askingforzip-input" type="text" name="zip_location" id="exportzipcode" style="color: black; height: 40px; border: 0px;" value="">
                </div>
                <div class="begex-useac-askingforzip-okbutton-container" style="float: left">
                    <input type="submit" value="Go" style="color: white; border: 0px; font-size: 17px; text-align: center; background: #15567B; color: white; padding: 10px; cursor: pointer; display: block;" >
                </div>
            </form>

            <!-- <div class="microsites-sidebarresults-results-container">
                 <?php print views_embed_view('state_resource_view', 'state_resource_resource_centers', $_GET['state']); ?>
                </div> -->
            <div class="microsites-sidebarresults-results-container-resource">

             </div>
        </div>

    </div>



    <!--asdjnkasndasndkasnknaslnslnasldnaklnaslnaslnaldks-->

  <!--  <div class="begex-useac-askingforzip">
        <div class="begex-useac-askingforzip-welcomemessage" style="clear: both; padding: 15px 0px;">
            Enter you ZIP Code to locate the closest U.S. Export Assistance Centers near you.
        </div>
        <form class="begex-useac-askingforzip-inputokbutton-container" style="overflow: hidden; padding-bottom: 25px;" onsubmit="getUseacViewForZipCode( jQuery('.begex-useac-askingforzip-input').val() ); return false;">
            <div class="begex-useac-askingforzip-input-container" style="float: left;">
                <input class="begex-useac-askingforzip-input" type="text" name="zip_location" id="exportzipcode1" style="color: black; height: 40px; border: 0px;" value="">
            </div>
            <div class="begex-useac-askingforzip-okbutton-container" style="float: left">
                <input type="submit" value="Go" style="color: white; border: 0px; font-size: 17px; text-align: center; background: #15567B; color: white; padding: 10px; cursor: pointer; display: block;" >
            </div>
        </form>
    </div>-->

    <script>
        getUserZipCode( function (zipCode) {
            jQuery('.begex-useac-askingforzip-input').val(zipCode);
            getUseacViewForZipCode(zipCode);
        });

        function getUseacViewForZipCode(zipCode) {

            // Show spinner while loading
            jQuery('.microsites-sidebarresults-results-container-resource').html('<img style="height: 35px; float: left" alt="" src="data:image/gif;base64," />');
            jQuery('.begex-useac-askingforzip').hide();

            consoleLog('Will get the View concerning the ZipCode of ' + zipCode + ', but first looking up the state that ZIpCode resides in...');
            phpFunction('getLatLongFromZipCode', zipCode, function (locationData) {
                var targView = 'state_resource_view';
                var targDisplay = 'state_resource_resource_centers';
                var state = locationData['state'];

                //var capZip = getCapitalZipcodebyState(getUrlVars()["state"]);

                if (state == getUrlVars()["state"].toUpperCase() ||  (state != getUrlVars()['state']))
                {
                    jQuery('.begex-useac-askingforzip-input').val(zipCode);

                }



                consoleLog('The target state is ' + state + ', now getting the USEACs View');
                jQuery.get('/sys/ajax/views_embed_view?view=' + targView + '&display=' + targDisplay + '&param1=' + state, function (viewHTML) {
                    jQuery('.microsites-sidebarresults-results-container-resource').hide();
                    jQuery('.microsites-sidebarresults-results-container-resource').html(viewHTML);
                    jQuery('.microsites-sidebarresults-results-container-resource').fadeIn();
                    jQuery('.begex-useac-askingforzip').fadeIn();
                    //In the side bars - display only 3 results with 'view more' option
                    $(".microsites-sidebarresults-results-container-resource .view-content" ).each(function(){

                        if(jQuery(this).find("div.views-row").length > 3){
                            // show the first three items
                            jQuery(this).find("div.views-row").filter(':gt(2)').hide();
                            // add view more link

                            $(this).append('<a id="viewall_resource" onclick="updateZipcode()"><div class="viewmore"><span>View All</span></div></a>');
                        }
                    });

                });
            });
        }


        function updateZipcode()
        {
            $('#viewall_resource').attr('href', '/request-appointment-and-closest-resource-centers?zip='+ $('#exportzipcode').val());


        }


    </script>





</div>

    <script>

        $(document).ready(function(){
            $('.sidebar-resource-centers').find('.viewmore').parent().attr('href', '/request-appointment-and-closest-resource-centers?&zip=' + $('#exportzipcode').val());


            setTimeout(function(){

            jQuery('#titleWrapper h1').text(AcronymntoStatename(getUrlVars()["state"]) + ' State Resources');
            $('#titleWrapper h1').css('textTransform', 'capitalize');

            $('.sidebar-top-web-resources div:eq(2)').css('display', 'block');
            $('.sidebar-top-web-resources').find('.microsite-sidebar-title-container').addClass("expanded")
            var capZip = getCapitalZipcodebyState(getUrlVars()["state"]);
            $('.begex-useac-askingforzip-input').val(capZip);
                getUseacViewForZipCode(capZip);

            }, 1000);

            //Event View all link
            $('.sidebar-events').find('.viewmore').parent().attr('href','/events/?f[0]=bundle%3Aevent');

            //Resource Center View all link (Zip code os respective state capital)

            //hide empty results and the corresponding title
            $('.microsite-sections-container').find('.view-empty').hide();
            $('.view-empty').parents('.microsite-section').hide();
            $('.view-empty').parents('.microsite-section-first').hide();



        });

        function getCapitalZipcodebyState(state)
        {
            var capZipCode = '';
            switch (state.toUpperCase())
            {
                case 'AL':
                {
                    capZipCode = '36101';
                    break;
                }
                case 'AK':
                {
                    capZipCode = '99801';
                    break;
                }
                case 'AS':
                {
                    capZipCode = '96799';
                    break;
                }
                case 'AZ':
                {
                    capZipCode = '85001';
                    break;
                }
                case 'AR':
                {
                    capZipCode = '72201';
                    break;
                }
                case 'CA':
                {
                    capZipCode = '94203';
                    break;
                }
                case 'CO':
                {
                    capZipCode = '80201';
                    break;
                }
                case 'CT':
                {
                    capZipCode = '06101';
                    break;
                }
                case 'DE':
                {
                    capZipCode = '19901';
                    break;
                }
                case 'DC':
                {
                    capZipCode = '20006';
                    break;
                }
                case 'FL':
                {
                    capZipCode = '32301';
                    break;
                }
                case 'GA':
                {
                    capZipCode = '30301';
                    break;
                }

                case 'GU':
                {
                    capZipCode = '96910';
                    break;
                }
                case 'HI':
                {
                    capZipCode = '96801';
                    break;
                }
                case 'ID':
                {
                    capZipCode = '83701';
                    break;
                }
                case 'IL':
                {
                    capZipCode = '62701';
                    break;
                }
                case 'IN':
                {
                    capZipCode = '46201';
                    break;
                }
                case 'IA':
                {
                    capZipCode = '50301';
                    break;
                }
                case 'KS':
                {
                    capZipCode = '66601';
                    break;
                }
                case 'KY':
                {
                    capZipCode = '40601';
                    break;
                }
                case 'LA':
                {
                    capZipCode = '70801';
                    break;
                }
                case 'ME':
                {
                    capZipCode = '04330';
                    break;
                }

                case 'MD':
                {
                    capZipCode = '21401';
                    break;
                }
                case 'MA':
                {
                    capZipCode = '02108';
                    break;
                }
                case 'MI':
                {
                    capZipCode = '48901';
                    break;
                }
                case 'MN':
                {
                    capZipCode = '55101';
                    break;
                }
                case 'MS':
                {
                    capZipCode = '39201';
                    break;
                }
                case 'MO':
                {
                    capZipCode = '65101';
                    break;
                }
                case 'MT':
                {
                    capZipCode = '59601';
                    break;
                }
                case 'NE':
                {
                    capZipCode = '68501';
                    break;
                }
                case 'NV':
                {
                    capZipCode = '89701';
                    break;
                }

                case 'NH':
                {
                    capZipCode = '03301';
                    break;
                }
                case 'NJ':
                {
                    capZipCode = '08601';
                    break;
                }
                case 'NM':
                {
                    capZipCode = '87501';
                    break;
                }
                case 'NY':
                {
                    capZipCode = '12201';
                    break;
                }
                case 'NC':
                {
                    capZipCode = '27601';
                    break;
                }
                case 'ND':
                {
                    capZipCode = '58501';
                    break;
                }
                case 'MP':
                {
                    capZipCode = '96950';
                    break;
                }
                case 'OH':
                {
                    capZipCode = '43201';
                    break;
                }
                case 'OK':
                {
                    capZipCode = '73101';
                    break;
                }
                case 'OR':
                {
                    capZipCode = '97301';
                    break;
                }

                case 'PA':
                {
                    capZipCode = '17101';
                    break;
                }
                case 'PR':
                {
                    capZipCode = '00901';
                    break;
                }

                case 'RI':
                {
                    capZipCode = '02901';
                    break;
                }
                case 'SC':
                {
                    capZipCode = '29201';
                    break;
                }
                case 'SD':
                {
                    capZipCode = '57501';
                    break;
                }
                case 'TN':
                {
                    capZipCode = '37201';
                    break;
                }
                case 'TX':
                {
                    capZipCode = '73301';
                    break;
                }
                case 'UT':
                {
                    capZipCode = '84101';
                    break;
                }
                case 'VT':
                {
                    capZipCode = '05601';
                    break;
                }
                case 'VI':
                {
                    capZipCode = '00802';
                    break;
                }
                case 'VA':
                {
                    capZipCode = '23218';
                    break;
                }
                case 'WA':
                {
                    capZipCode = '98501';
                    break;
                }
                case 'WV':
                {
                    capZipCode = '25301';
                    break;
                }

                case 'WI':
                {
                    capZipCode = '53701';
                    break;
                }
                case 'WY':
                {
                    capZipCode = '82001';
                    break;
                }
                default:
                {
                    capZipCode ='';
                }
            }
            return capZipCode;
        }


        function AcronymntoStatename(accr)
        {
            var statefullname = '';
            switch (accr.toUpperCase())
            {
                case 'AL':
                {
                    statefullname = 'Alabama';
                    break;
                }
                case 'AK':
                {
                    statefullname = 'Alaska';
                    break;
                }
                case 'AS':
                {
                    statefullname = 'American Samoa';
                    break;
                }
                case 'AZ':
                {
                    statefullname = 'Arizona';
                    break;
                }
                case 'AR':
                {
                    statefullname = 'Arkansas';
                    break;
                }
                case 'CA':
                {
                    statefullname = 'California';
                    break;
                }
                case 'CO':
                {
                    statefullname = 'Colorado';
                    break;
                }
                case 'CT':
                {
                    statefullname = 'Connecticut';
                    break;
                }
                case 'DE':
                {
                    statefullname = 'Delaware';
                    break;
                }
                case 'DC':
                {
                    statefullname = 'District of Columbia';
                    break;
                }
                case 'FL':
                {
                    statefullname = 'Florida';
                    break;
                }
                case 'GA':
                {
                    statefullname = 'Georgia';
                    break;
                }

                case 'GU':
                {
                    statefullname = 'Guam';
                    break;
                }
                case 'HI':
                {
                    statefullname = 'Hawaii';
                    break;
                }
                case 'ID':
                {
                    statefullname = 'Idaho';
                    break;
                }
                case 'IL':
                {
                    statefullname = 'Illinois';
                    break;
                }
                case 'IN':
                {
                    statefullname = 'Indiana';
                    break;
                }
                case 'IA':
                {
                    statefullname = 'Iowa';
                    break;
                }
                case 'KS':
                {
                    statefullname = 'Kansas';
                    break;
                }
                case 'KY':
                {
                    statefullname = 'Kentucky';
                    break;
                }
                case 'LA':
                {
                    statefullname = 'Louisiana';
                    break;
                }
                case 'ME':
                {
                    statefullname = 'Maine';
                    break;
                }

                case 'MD':
                {
                    statefullname = 'Maryland';
                    break;
                }
                case 'MA':
                {
                    statefullname = 'Massachusetts';
                    break;
                }
                case 'MI':
                {
                    statefullname = 'Michigan';
                    break;
                }
                case 'MN':
                {
                    statefullname = 'Minnesota';
                    break;
                }
                case 'MS':
                {
                    statefullname = 'Mississippi';
                    break;
                }
                case 'MO':
                {
                    statefullname = 'Missouri';
                    break;
                }
                case 'MT':
                {
                    statefullname = 'Montana';
                    break;
                }
                case 'NE':
                {
                    statefullname = 'Nebraska';
                    break;
                }
                case 'NV':
                {
                    statefullname = 'Nevada';
                    break;
                }

                case 'NH':
                {
                    statefullname = 'New Hampshire';
                    break;
                }
                case 'NJ':
                {
                    statefullname = 'New Jersey';
                    break;
                }
                case 'NM':
                {
                    statefullname = 'New Mexico';
                    break;
                }
                case 'NY':
                {
                    statefullname = 'New York';
                    break;
                }
                case 'NC':
                {
                    statefullname = 'North Carolina';
                    break;
                }
                case 'ND':
                {
                    statefullname = 'North Dakota';
                    break;
                }
                case 'MP':
                {
                    statefullname = 'Northern Mariana Islands';
                    break;
                }
                case 'OH':
                {
                    statefullname = 'Ohio';
                    break;
                }
                case 'OK':
                {
                    statefullname = 'Oklahoma';
                    break;
                }
                case 'OR':
                {
                    statefullname = 'Oregon';
                    break;
                }

                case 'PA':
                {
                    statefullname = 'Pennsylvania';
                    break;
                }
                case 'PR':
                {
                    statefullname = 'Puerto Rico';
                    break;
                }
               case 'RI':
                {
                    statefullname = 'Rhode Island';
                    break;
                }
                case 'SC':
                {
                    statefullname = 'South Carolina';
                    break;
                }
                case 'SD':
                {
                    statefullname = 'South Dakota';
                    break;
                }
                case 'TN':
                {
                    statefullname = 'Tennessee';
                    break;
                }
                case 'TX':
                {
                    statefullname = 'Texas';
                    break;
                }
                case 'UT':
                {
                    statefullname = 'Utah';
                    break;
                }
                case 'VT':
                {
                    statefullname = 'Vermont';
                    break;
                }
                case 'VI':
                {
                    statefullname = 'Virgin Island';
                    break;
                }
                case 'VA':
                {
                    statefullname = 'Virginia';
                    break;
                }
                case 'WA':
                {
                    statefullname = 'Washington';
                    break;
                }
                case 'WV':
                {
                    statefullname = 'West Virginia';
                    break;
                }

                case 'WI':
                {
                    statefullname = 'Wisconsin';
                    break;
                }
                case 'WY':
                {
                    statefullname = 'Wyoming';
                    break;
                }
                default:
                {
                    statefullname ='';
                }
            }
            return statefullname;
        }



    </script>
