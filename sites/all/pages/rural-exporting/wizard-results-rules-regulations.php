...
// Enabel debug message if the current user is the administrator
	...
	// Load potential results from excel
		$potentialResults = ya_wizard_excelToArray('sites/all/pages/rural-exporting/wizard-results-rules-regulations.xls');
		...
		foreach ( $potentialResults as $key => $potentialResult ) {
			...
			$anyFromLastSectionIsBothOfInterestAndSelected = intval(
				$animal || $grains || $livestock || $meat_poultry || $plants || $regulation_yes || $research_yes || $export_no || $international_buyer_yes
			);
