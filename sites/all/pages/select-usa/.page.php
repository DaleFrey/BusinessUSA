<?php ob_start(); ?>

<?php

    print theme(
            'yawizard_from_excel',
            array(
                'path' => overridable('sites/all/pages/select-usa/wizard-questions.xls'),
                'resultsURL' => '/select-usa/wizard-results'
            )
    );

?>

<!-- The following style block is stored at <?php print __FILE__; ?> -->
<style rendersource="<?php print basename(__FILE__); ?>">
    /* The following style tag is stored in <?php print __FILE__; ?> */

    /* Custom styles for show the email-button, and hide the next button when slide-5 is active */
    /* This markup for div.wizard-emailbtn-link is stored in wizard-abstract.tpl.php and is always hidden (by global.css) unless forced-visible otherwise */
    .wizard-container.wizard-state-slide-5 div.emailbtn-container {
        display: block;
    }
    .slide-id-5 .wizard-continue,
    .wizard.current-slide-is-5 .wizard-next {
        display: none !important;
    }

    /* Always hide this checkbox, this will be selected by javascript */
    .question-container.wiztag-email_filled {
        display: none !important;
    }

    body.page-contains-wizard .questions .question-controle.wiztag-whereisthislocation,
    body.page-contains-wizard .questions .question-controle.wiztag-inwhichcountryisyourbusinesslocated {
      display: block;
    }

</style>

<script rendersource="<?php print basename(__FILE__); ?>">

    /* The following script tag is stored at <?php print __FILE__; ?> */

    /* Custom script for this wizard to trigger the email-form through AJAX */
    jQuery(document).ready( function () {

        setInterval( function () {
          if (typeof(selectUSA_custom_damon) == "function") {
            selectUSA_custom_damon();
          }
        }, 250);

        // When the user clicks on the button (on the wizard-splash screen) to show the Wizard...
        jQuery('.wizard-welcomemessage-startbtn').bind('click', function () {
            // ...bind a click event for when the user clicks on the "No" option for the tag ident_location_interest_no...
            jQuery('[wiztag=ident_location_interest_no]').parents('.radio-wrapper').bind('click', function () {
                // ...clear the checked-values for the checkboxes on this slide (slide 2)
                jQuery('.questions-for-step-2-container input[type=checkbox]').removeAttr('checked');
                jQuery('.questions-for-step-2-container .wizard-checkbox-lable.label_check').removeClass('c_on');
                // and force update the wizard_selected_tags variable
                checkboxClickEventHandelerForTagging(true);
            });
        });

        // When the user clicks on the "Send" label that is only visible in the mobile layout on the "Contact Us" slide...
        jQuery('.wizard-uniqueid-selectusa .wizard-question-text.wizard-tag-contact_send_labelbtn').bind('click', function () {
            // ...trigger this function defined in wizard-abstract.tpl.php
            yawizard.next()
        });

        jQuery('#wiztag-contact_send_labelbtn').bind('click', function () {

            var colorboxContent = '<div class="selectusa-contactus-colorbox" style="min-width: 350px; min-height: 75px;"><img style="padding-right: 20px; height: 35px; float: left" alt="" src="data:image/gif;base64," /> Sending contact request...</div>';
            jQuery.colorbox({
                html: colorboxContent,
                overlayClose: false,
                escKey: false,
                onLoad: function () {
                    jQuery('#cboxClose').remove();
                }
            });

            jQuery.post('/select-usa/contact-us-receiver', yawizard.getAllTags(), function (responceHTML) {
                jQuery('.selectusa-contactus-colorbox').html(responceHTML);
            });
        });

        // Custom JavaScript for the SelectUSA/"Invest in the USA" wizard

            // When the user clicks the "Is your business located within the U.S" checkbox, reset the values of the questions which (visibility wise) depends on this question
            jQuery('.wizard-container.wizard-uniqueid-selectusa .wizard-tag-business_is_in_usa_yes').bind('click', function () { /* Coder Bookmark: CB-DX08AOM-BC */
                jQuery('input.wizard-tag-location_location_textbox').val('or type a location');
                jQuery('.wizard-question-step-id-1.wizard-question-question-id-2 select').val('Select a State');
                jQuery('.wizard-question-step-id-1.wizard-question-question-id-2 select').change();
            });

            // When the user clicks the "Is your business located outside the U.S" checkbox, reset the values of the questions which (visibility wise) depends on this question
            jQuery('.wizard-container.wizard-uniqueid-selectusa .wizard-tag-business_is_in_usa_no').bind('click', function () { /* Coder Bookmark: CB-WDYLJI6-BC */
                jQuery('.wizard-question-step-id-1.wizard-question-question-id-5 select').val('Select a State');
                jQuery('.wizard-question-step-id-1.wizard-question-question-id-5 select').change();
            });

            // When the user clicks the "Have you identified one or more U.S. locations you are interested in locating your investment" checkbox, reset the values of the questions which (visibility wise) depends on this question
            jQuery('.wizard-container.wizard-uniqueid-selectusa .wizard-question-step-id-2.wizard-question-question-id-0').bind('click', function () { /* Coder Bookmark: CB-LLSCB72-BC */
                jQuery('.wizard-question-step-id-2.wizard-question-question-id-1 select').val('Select a State');
                jQuery('.wizard-question-step-id-2.wizard-question-question-id-1 select').change();
            });

            // When the textbox under "Please enter the location" gain focus, make the "or type a location" text dissappear.
            jQuery('.wizard-container.wizard-uniqueid-selectusa input.wizard-tag-location_location_textbox, .wizard-container.wizard-uniqueid-selectusa input.wizard-tag-investment_location_textbox, .wizard-container.wizard-uniqueid-selectusa input.wiztag-hidemeshowonlythetextbox').bind('focus', function () {
                /* Coder Bookmark: CB-4TBNE77-BC */
                var jqThis = jQuery(this);
                if ( jqThis.val() == 'or type a location' ) {
                    jqThis.val('');
                    jqThis.css('color', 'black')
                }
            });

            // When the textbox under "Please enter the location" losses focus, make the "or type a location" text reappear when nessesary.
            jQuery('.wizard-container.wizard-uniqueid-selectusa .wizard-tag-location_location_textbox, .wizard-container.wizard-uniqueid-selectusa input.wizard-tag-investment_location_textbox, .wizard-container.wizard-uniqueid-selectusa input.wiztag-hidemeshowonlythetextbox').bind('blur', function () {
                /* Coder Bookmark: CB-WG20JJA-BC  */
                var jqThis = jQuery(this);
                if ( jQuery.trim(jqThis.val()) == '' ) {
                    jqThis.val('or type a location')
                    jqThis.css('color', '#696969')
                }
            });

        //Move 'or type a location' textbox beside the select state dropdown.

            jQuery('.slide-id-2 .question-controle-for-wiztag-pleaseenterthelocation').append(jQuery('.slide-id-2').find('#wiztag-hidemeshowonlythetextbox'));

            jQuery('.slide-id-1 .question-controle-for-wiztag-whereisthislocation').append(jQuery('.slide-id-1').find('#wiztag-hidemeshowonlythetextbox'));



      //  jQuery('.question-controle-for-wiztag-whereisthislocation').append(jQuery('#wiztag-hidemeshowonlythetextbox'));



    });

</script>

<?php
    if ( !empty($_GET['widget']) && intval($_GET['widget']) === 1 ) {
        global $overrideBodyMarkup;
        $overrideBodyMarkup = ob_get_contents();
        ob_end_clean();
    } else {
        ob_end_flush();
    }
?>
