...
<div class="search-export-dialogue" style="display: none;" >
    ...
    <div class="vendorLightBox-logo" >
        <img id="cbox-logo" src="/sites/all/themes/bizusa/images/logo.png" alt="Business USA Logo">
    ...
    <div class="vendorLightBox-msg-area">
        <form method="post" action="/download_search_xls2" id="search-export-form" onsubmit="return validateExportForm();">
            <table>
                <tr>
                    <th>Page Selection</th>
                </tr>
                <tr>
                    <?php if($variables['response']->response->numFound > 10) { ?>
                    <td><input type="radio" name="dc" value="<?php echo 'current_' . $_REQUEST['page']; ?>" checked="true" />&nbsp;Current Page</td>
                        <?php
                            if($variables['response']->response->numFound <= 100 && $variables['response']->response->numFound > 10){
                                print '<td><input type="radio" name="dc" value="'. $variables['response']->response->numFound .'"/>&nbsp;All Pages</td>';
                            }
                        ?>
                    <?php }else if($variables['response']->response->numFound <= 10){
                                print '<td><input checked="true" type="radio" name="dc" value="'. $variables['response']->response->numFound .'"/>&nbsp;All Search Results</td>';
                        } ?>
                </tr>
                <?php if($variables['response']->response->numFound > 10){?>
                    <tr>
                         <td colspan="2">
                             ...
                             <span id="export-message">(Max allowed pages: 10)</span>
                         </td>
                    </tr>
                <?php } ?>
            ...
            <textarea type="hidden" id="no_of_pages"  class="never-show-ever"><?php echo ceil(intval($variables['response']->response->numFound)/10); ?></textarea>
            <textarea type="hidden" class="never-show-ever params_serialized" name="params_serialized" id="params_serialized"><?php echo serialize( $variables['query']->getSolrParams() ); ?></textarea>
            <input type="submit" class="export-to-excel" value="Download"
                   DevNote="this link targets a Basic Page stored in the Drupal database" />&nbsp;&nbsp;
            <input type="button" id="export_dialogue_close" value="Close" onclick="close_export_dialogue();"/>
        </form>
    </div>
</div>
...
<!-- Start: Search Across Business Agencies /* Coder Bookmark: CB-OHNKFA5-BC */ -->
<?php print theme('search_results_search_across_business_agencies', array('search' => $search_term)); ?>
<!-- End Coder Bookmark: CB-OHNKFA5-BC -->
